...
> **Draft for review.** Dates and lead times are drawn from the current
> instructional articles. **Two retention schedules are in effect, both
> published, and they do not agree** — the largest open question in this set.
>
> - **Decision needed — which retention schedule governs.** The quarter-based
>   schedule (`KB0034559`) keeps access for one additional quarter, ignoring
>   Summer for Spring, and purges the environment one quarter after
>   deactivation. The day-based schedule (`KB0030588`) closes the course ticket
>   at 11 days, removes shared nbgrader/TA access at 45 days, and removes
>   instructor and student access at 90 days, counted from the last day of class.
>   For Fall and Winter courses the two land within a few weeks of each other.
>   **For Spring courses they are irreconcilable**: 90 days after a Spring course
>   ends falls in Summer, while the quarter-based rule skips Summer entirely and
>   holds access to the end of Fall — a difference of months. A human has to
>   decide which is policy; whichever loses needs correcting at its published
>   source.
> - **Decision needed — the 45-day milestone has no counterpart.** Nothing in the
>   quarter-based schedule removes shared nbgrader/TA access before anyone
>   else's. If the 45-day milestone is real, grading, regrades and hand-off have
>   a much earlier deadline than the quarter rule implies, and it is not
>   documented anywhere a TA would look.
> - **Decision needed — the interim advice.** Until this is settled the page
>   tells readers to act on the earlier of the two dates. That is a writer's
>   stopgap for reader safety, not a policy statement.
> - **Settled 2026-08-28 — the roster timings.** `KB0034559` is correct: rosters
>   are loaded into workspaces **one business day before the start of the term**,
>   and a TSS change is reflected **by 10am the day following the change**.
>   `KB0030470`'s "allow 1-2 business days" is wrong and needs correcting at
>   source.
> - **Missing:** no published lead time for **independent study or research
>   access**. We tell people to submit the form; we do not tell them how long
>   provisioning takes, and it is the single most common follow-up question.
>   <!-- FIGURE: typical days from Independent Study Request submission to first
>        sign-in -->
> - **Missing:** nothing is published about what a new member receives on the
>   Fall 2026 reservation system when access opens — whether a Service Unit
>   budget is in place from day one, and whether it is prorated for a late add.
>   Managers will ask this in week 1.
> - **Missing:** what happens to a member's cluster-wide `private/` directory and
>   its personal quota when the last course workspace closes. Nothing published
>   says whether it persists, and students ask.
> - **Check before publishing:** the Independent Study Request form replaced the
>   CINFO process (fact base §4.14). The body no longer says so, since system
>   history does not belong in a page, but articles still directing readers to
>   CINFO remain in circulation.
> - **Unverified:** `KB0030606` says trouble reports from independent study users
>   "may take up to 5 days" to answer, against the 1-2 business day target
>   published for individual issues generally. That article is stale in other
>   respects, so the figure is not repeated below; confirm it or retire it.
> - **Check before publishing:** the worked dates below are transcribed from the
>   key-dates table published in `KB0034559` and will need re-checking against
>   that table each cycle. Consider linking to it rather than restating it.
