...
| Best-effort reservation | A session admitted with no runtime guarantee and no charge. Not enabled on DSMLP. | [Best-Effort Reservations](../gpu-access/reservations.md#best-effort-reservations) |
...
| Claim window | The period at the start of a booked window by which a matching session must be waiting or running, or the booking is cancelled as a no-show. | [The Claim Window](../gpu-access/reservations.md#the-claim-window) |
...
| `ORG_ON_DEMAND` | The default workspace, for on-demand leases only, that a GPU session naming no course is charged to, such as a launch without `-W`. | [The Default Workspace](../gpu-access/reservations.md#the-default-workspace) |
...
| Reservation app | The web app at [`reserve.dsmlp.ucsd.edu`](https://reserve.dsmlp.ucsd.edu/) where GPU windows are booked, cancelled, and extended. | [The Reservation App](../gpu-access/reservations.md#the-reservation-app) |
| Reservation event | A Kubernetes event the reservation system writes on a GPU pod, such as `OnDemandLeaseDenied` or `Preempted`. | [Reservation Events](reservation-events.md) |
...
| Runtime guarantee | The period in which a session cannot be preempted: to the end of its reservation, and through any booking that directly follows it. | [End of a Reservation Window](../gpu-access/what-ends-a-session.md#end-of-a-reservation-window) |
...
| Senior and junior borrowing | The two borrowing tiers. A junior borrower may borrow only the idle capacity above a deeper reserve floor; the tier applies only when a reservation is made. | [Borrowing Beyond Quota](../gpu-access/quotas-and-availability.md#borrowing-beyond-quota) |
...
| Team mode | A reservation feature in which a team shares one Service Unit budget, and teammates can see, cancel, and adopt one another's bookings. | [Team Mode](../gpu-access/reservations.md#team-mode) |
